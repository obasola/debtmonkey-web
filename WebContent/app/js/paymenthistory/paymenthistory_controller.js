...
paymentHistoryModule.controller('PaymentHistoryCtrl', ['PaymentHistory', 'Account', '$scope', '$routeParams', '$http', '$location', '$cookies', 'MessageHandler', 'restURL', function (PaymentHistory, Account, $scope, $routeParams, $http, $location, $cookies, MessageHandler, restURL) {
        ...
        $scope.loadAccountList = function() {
            ...
            $location.path('/scheduleHistories');
        };
        ...
        /**
         * Refresh account payment history
         */
        $scope.loadHistoryRecords =  function(accountId) {
        	$scope.selectedAccount = accountId;
            $scope.refreshAccountHistory(accountId);
        };
        
        $scope.refreshAccountHistory = function (id) {
            try {
                $scope.historyEntries = null;
                PaymentHistory.getAccountHistory(id).then(
                        function (success) {
                            $scope.historyEntries = success.data;
                        },
                        MessageHandler.manageError);
            } catch (ex) {
                MessageHandler.manageException(ex);
